...
from finetune.finetune_utils import train_one_epoch, run_eval_reg, save_finetune_ckpt
...
def parse_args():
    ...
    # basic
    parser.add_argument('--dataroot', type=str, default="../datasets/fine-tuning/pdbbind", help='data root')
    ...
    parser.add_argument('--split_type', default="30", type=str, help='', choices=["30", "60", "scaffold"])
    ...
    # log
    parser.add_argument('--log_dir', default='./experiments/finetune_pdbbind', help='path to log')
...
def main(local_rank, ngpus_per_node, args):
    ...
    valid_dataset = PocketWithLigandImageDataset(args.dataroot, split_type=args.split_type, split="valid",center=args.center, img_transform=transforms, max_len_pocket=args.max_len_pocket)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_loss, train_results, train_desc_results = run_eval_reg(vision_model, gnn_model, fusion_model, predictor, train_loader, criterion, device, epoch, get_y_pred=get_y_pred_func, tqdm_desc=tqdm_eval_train_desc)
        valid_loss, valid_results, valid_desc_results = run_eval_reg(vision_model, gnn_model, fusion_model, predictor, valid_loader, criterion, device, epoch, get_y_pred=get_y_pred_func, tqdm_desc=tqdm_eval_val_desc)
        test_loss, test_results, test_desc_results = run_eval_reg(vision_model, gnn_model, fusion_model, predictor, test_loader, criterion, device, epoch, get_y_pred=get_y_pred_func, tqdm_desc=tqdm_eval_test_desc)

        if lr_scheduler is not None:
            lr_scheduler.step(valid_results["RMSE"])
        ...
        if is_left_better_right(valid_results["RMSE"], results["highest_valid"]["RMSE"], standard="min"):
            results['epoch'] = epoch
