...
if __name__ == '__main__':
    ...
    # ProtMD 使用的参数
    net = EGNN_Network(num_tokens=100, dim=128, depth=6, num_nearest_neighbors=32, dropout=0.15,
                       global_linear_attn_every=1, norm_coors=True, coor_weights_clamp_value=2)

    n_batch = 4
    max_len = 512
    feats = torch.randint(0, 21, (n_batch, max_len))  # (1, 1024)
    coors = torch.randn(n_batch, max_len, 3)  # (1, 1024, 3)
    mask = torch.ones_like(feats).bool()  # (1, 1024)

    feats_out, coors_out = net(feats, coors, mask=mask)  # (1, 1024, 32), (1, 1024, 3)


    from dataloader.dataset import PocketWithLigandImageDataset
    from torch.utils.data import DataLoader
    from torchvision import transforms
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    transform = transforms.Compose([transforms.RandomResizedCrop(224), transforms.ToTensor(), normalize])
    dataroot = "D:/xianghongxin/work/projects/MDMolKD/MDMolKD/datasets/pre-training/MISATO-10-complex/processed/"
    dataset = PocketWithLigandImageDataset(dataroot, center=True, img_transform=transform, max_len_pocket=100)
    dataloader = DataLoader(dataset, batch_size=8, collate_fn=dataset.collate_fn)
    for data in dataloader:
        # if (data["pymol_2d_m"] == 0).any():
        #     pass
        # print(data)
        feats_out, coors_out = net(data["pocket_feats"], data["pocket_coords"], mask=data["mask"])
        global_feats = (feats_out * data["mask"].long().resize(8, 100, 1)).sum(1) / torch.unsqueeze(data["mask"].long().sum(1), dim=1)

        print(feats_out.shape, coors_out.shape, global_feats.shape)
